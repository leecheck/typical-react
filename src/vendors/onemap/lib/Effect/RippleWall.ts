...
export interface RippleWallOption {
  height?: number;
  elevation?: number;
  color?: string;
  opacity?: number;
...
export default class RippleWall extends BaseObject {
   ...
   * @param {*} layer
   */
  constructor(poly: [], options: RippleWallOption, layer: ThreeLayer) {
    ...
    if (poly.length > 0) {
      ...
      poly.pop();
      poly.push([last[0] + 0.0000001, last[1] + 0.0000001]);
    }
  ...
  createGeometry(polygon, layer, height) {
    ...
    const positionsV: number[] = [];
    let joinLonLat: number[] = [];
    wall.forEach((lnglat) => {
    ...
    for (var a = joinLonLat, polySub = [], o = 0, s = 0; o < a.length - 2; o += 2, s++)
      0 === o
        ...
        : (polySub[s] =
            polySub[s - 1] +
            Math.sqrt(
              (a[o + 2] - a[o]) * (a[o + 2] - a[o]) + (a[o + 3] - a[o + 1]) * (a[o + 3] - a[o + 1]),
            ));
    let pos = [],
      uvs = [];
  ...
  _animation() {
    ...
    wall.material.uniforms.time.value += 0.015;
  }
}
